...
export default function HeaderButton() {
            ...
            default:
                return currentUser ?
                    <Link className={styles.link} to="/user">
                        <button aria-label="go User" className={`${styles.button} ${styles.customer} ${styles.active}`}>
                            <svg className={styles.svg} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                <path d="M224 256c70.7 0 128-57.3 128-128S294.7 0 224 0S96 57.3 96 128s57.3 128 128 128zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304H178.3z" />
                            </svg>
                            <span className={styles.name}>{currentUser.uid.slice(0, 10)}</span>
                        </button>
                    </Link> :
                    <Link className={styles.link} to="/membership/login" >
                        <button aria-label="go Membership" className={`${styles.button} ${styles.customer}`}>
                            <svg className={styles.svg} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
                                <path d="M224 256c70.7 0 128-57.3 128-128S294.7 0 224 0S96 57.3 96 128s57.3 128 128 128zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512H418.3c16.4 0 29.7-13.3 29.7-29.7C448 383.8 368.2 304 269.7 304H178.3z" />
                            </svg>
                        </button>
    ...
    return (
        <div className={styles.HeaderButton}>
            <button onClick={handleShowHeaderSearch} onMouseLeave={handleHideHeaderSearch} aria-label="show Search" className={`${styles.button} ${styles.search}`}>
            ...
            <button aria-label="show Cartbar" style={{ "--x": `${renderCartbarNumber()}` }} onClick={handleShowCartbar} className={`${styles.button} ${styles.cart} ${cartbarItem.length === 0 ? "" : `${styles.active}`}`}>
            ...
            <button aria-label="show Sidebar" onClick={handleShowSidebar} className={`${styles.button} ${styles.hamburger}`}>
                <svg className={styles.svg} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
