...
export const cartbarItemSlice = createSlice({
    ...
    reducers: {
        ...
        addToCartbarItem: (state, data) => {
            const { cartbarItem, findResult, color, size, number, maxNumber } = data.payload
            ...
            const foundDimension = newFindResult.dimension.find(item => item.size === size);
            if (foundDimension) {
...
export const { resetCartbarItem, addToCartbarItem, decrementCartbarItem, incrementCartbarItem, deleteCartbarItem } = cartbarItemSlice.actions
